const instance = axios.create({
  ...
  async function getAllData() {
    ...
    try {
      const res = await instance.get("/voiture");
      const result = {
  ...
  async function getDataById() {
    ...
    if (id) {
      try {
        const res = await instance.get(`/voiture/${id}`);
        const result = {
          status: res.status + "-" + res.statusText,
          headers: res.headers,
          data: res.data,
  ...
  async function getDataByTitle() {
    ...
    if (title) {
      try {
        // const res = await instance.get(`/voiture?title=${title}`);
        const res = await instance.get("/voiture", {
          params: {
            title: title
          }
  ...
  async function postData() {
    ...
    try {
      const res = await instance.post("/voiture", {
        title: title,
  ...
  async function putData() {
    ...
    try {
      const res = await instance.put(`/voiture/${id}`, {
        title: title,
        description: description,
        published: published
      });
  ...
  async function deleteAllData() {
    ...
    resultElement.innerHTML = "";
    try {
      const res = await instance.delete("/voiture");
      const result = {
        status: res.status + "-" + res.statusText,
        headers: res.headers,
        data: res.data,
  ...
  async function deleteDataById() {
    ...
    try {
      const res = await instance.delete(`/voiture/${id}`);
      const result = {
        status: res.status + "-" + res.statusText,
        headers: res.headers,
        data: res.data,
